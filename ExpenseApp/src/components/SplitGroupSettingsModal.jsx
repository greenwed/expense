...
export default function SplitGroupSettingsModal({
  ...
  const loadSuggestions = React.useCallback(async () => {
    ...
    try {
      ...
      const list = await fetchCategorySuggestions(groupId, true);
      setSuggestions(list);
    } catch (err) {
      console.error('Failed to load split category suggestions:', err);
    } finally {
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <span className="text-xs text-slate-400 dark:text-slate-400">
                    Layer 1 Global Defaults & Layer 3 Split Group Custom
                  </span>
                </div>
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    await approveCategorySuggestion(s.id, { groupId, isSplit: true });
                                    await loadSuggestions();
                                    await fetchGroupCategories(groupId, true);
                                  } catch (err) {
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    setActionSuggestionId(s.id);
                                    await rejectCategorySuggestion(s.id, { groupId, isSplit: true });
                                    await loadSuggestions();
                                  } catch (err) {
              ...
              <div className="space-y-2">
                <h5 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  This Split Group's Categories ({getCustomCategories(groupId).length})
                </h5>
                {getCustomCategories(groupId).length === 0 ? (
                  <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-[#1A2234] border border-dashed border-slate-200 dark:border-slate-800 text-center text-xs text-slate-400">
                    No custom categories created yet. {isCreator ? 'Click "+ Add Category" to create one.' : 'Click "+ Suggest Category" to request one.'}
                  </div>
                ) : (
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    await deleteCategory(cat.id || cat._id, groupId, true);
                                  } catch (err) {

  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-backdrop-fade">
      <div className="bg-white border border-slate-200/80 rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-modal-pop">
        ...
        <div className="p-6 pb-4 shrink-0 flex items-center justify-between gap-3 bg-slate-50/50 border-b border-slate-100">
          <div>
            <span className="text-xs text-slate-400 font-semibold block">Total Monthly Expenses</span>
          ...
          {canManage && (
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-extrabold text-xs rounded-2xl shadow-md shadow-indigo-500/20 flex items-center gap-1.5 transition-all active:scale-95"
            >
        ...
        <div className="p-6 overflow-y-auto space-y-3 flex-1">
          {expenses.length === 0 ? (
            <div className="py-8 text-center text-slate-400 space-y-2">
              <Receipt className="w-10 h-10 stroke-[1.5] mx-auto text-slate-300" />
              <p className="text-xs font-bold text-slate-600">No expense entries recorded this month.</p>
              <p className="text-[11px] text-slate-400">Click "+ Add Expense" to record daily expenses!</p>
            </div>
          ) : (
            expenses.map((item) => {
              ...
              return (
                <div
                  ...
                  className="p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between gap-3 group hover:border-indigo-200 transition-colors"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-slate-400 flex-wrap">
                        ...
                        {item.userName && (
                          <span className="text-indigo-700 font-semibold">• by {item.userName}</span>
                        )}
                      </div>
                    </div>
                  </div>
